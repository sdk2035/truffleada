...
package body Exp_Ch9 is
   ...
   begin
      ...
      if Nkind (Op_Spec) = N_Function_Specification or else not Exc_Safe then
      ...
      Lock_Stmt := Make_Procedure_Call_Statement (Loc,
        Name => Lock_Name,
        Parameter_Associations => Object_Parm);
      ...
      if Abort_Allowed then
         Stmts := New_List (
           ...
           Lock_Stmt, Unprot_Call);

      else
         Stmts := New_List (Lock_Stmt, Unprot_Call);
      end if;

      if Nkind (Op_Spec) /= N_Function_Specification and then Exc_Safe then
         Unlock_Stmt := Make_Procedure_Call_Statement (Loc,
           Name => Unlock_Name,
           Parameter_Associations => Object_Parm);
   ...
   begin
      ...
      --  protected type, the internal and external subprograms follow each
      --  other on the entity chain. If the operation is defined in the private
      --  part of the type, there is no need for a separate locking version of
      --  the operation, and internal calls use the protected_body_subprogram
      --  directly.

      if List_Containing (Decl) = Private_Declarations (Parent (Decl)) then
         return Subp;
      else
   ...
   begin
      ...
      while Present (Op_Body) loop
         ...
         case Nkind (Op_Body) is
            ...
            when N_Subprogram_Body =>
               ...
               --  is a visible operation of the type, or if it is an
               --  interrupt handler. Otherwise it is only callable from within
               --  the object, and the unprotected version is sufficient.
                  ...
                  if Nkind (Parent (Op_Decl)) = N_Protected_Definition
                    and then
                      (List_Containing (Op_Decl) =
                        ...
                        or else
                         Is_Interrupt_Handler (Corresponding_Spec (Op_Body)))
                  then
                     New_Op_Body :=
                        Build_Protected_Subprogram_Body (
      ...
      if Has_Entries and then
        (Abort_Allowed or else Restrictions (No_Entry_Queue) = False
   ...
   begin
      ...
      if Delay_Count > 0 then
         ...
         begin
            ...
            elsif Is_RTE (Base_Type (Etype (Time_Type)), RO_RT_Time) then
   ...
   begin
      ...
      Pdef := Entity (Name (D_Stat));

      --  Get the delay expression from the one before the delay
      --  expansion is made.

      if Is_RTE (Pdef, RO_CA_Delay_For) then
         Disc := Make_Integer_Literal (Loc, 0);
         Conv := Relocate_Node (Expression (Original_Node (Node_Id (D_Stat))));

      elsif Is_RTE (Pdef, RO_CA_Delay_Until) then
         Disc := Make_Integer_Literal (Loc, 1);
         Conv := Make_Function_Call (Loc,
           New_Reference_To (RTE (RO_CA_To_Duration), Loc),
           New_List (New_Copy (Expression (Original_Node
             (Node_Id (D_Stat))))));
