...
package body Sem_Elim is
   ...
   procedure Check_Eliminated (E : Entity_Id) is
      ...
      procedure Eliminate_Subprogram (Subp : Entity_Id);
      --  Mark the subprogram and all its default expression functions.

      procedure Eliminate_Subprogram (Subp : Entity_Id) is
         Formal : Entity_Id;

      begin
         Set_Is_Eliminated (Subp);

         Formal := First_Formal (Subp);

         while Present (Formal) loop

            if Present (Default_Expr_Function (Formal)) then
               Set_Is_Eliminated (Default_Expr_Function (Formal));
            end if;

            Next_Formal (Formal);
         end loop;
      end Eliminate_Subprogram;
   ...
   begin
      ...
      Elmt := Elim_Hash_Table.Get (Get_Key (Data));
