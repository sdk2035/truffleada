         ...
         case Nam is
            ...
            when Name_Enclosing_Entity =>
               ...
               loop
                  ...
                  then
                     Ent := Defining_Unit_Name (P);
                     exit;

                  elsif Nkind (P) = N_Task_Body
                    or else Nkind (P) = N_Entry_Body
                    or else Nkind (P) = N_Protected_Body
                  then
                     Ent := Defining_Identifier (P);
                     exit;
                  end if;
         ...
         if Is_RTE (Pool, RE_SS_Pool) then
            ...
            if Is_Class_Wide_Type (Desig_T) then
               ...
               begin
                  ...
                  Set_Esize (Acc_Type, Esize (Typ));
                  Set_Alignment (Acc_Type, Alignment (Typ));
                  Set_Directly_Designated_Type (Acc_Type,
                    Entity (Make_Subtype_From_Expr (Deref, Desig_T)));
