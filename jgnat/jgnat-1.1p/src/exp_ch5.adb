...
package body Exp_Ch5 is
   ...
   begin
      ...
      --  We must have at least one slice at this stage. If either operand
      --  is bit packed, then we need a loop, since we can't be sure that
      --  the slice is byte aligned.
   ...
   begin
      ...
      --  of invalid scalar values.
         ...
         --  sure that the setting of this flag is correct.
            ...
            --  If we are in full validity checking mode, then we always
            --  check the validity of the right hand side on all assignments.
      ...
      --  standard validity checking mode. No need to do this if no
      --  validity checking, or if full validity checking (in the latter
      --  case all indexed components are fully checked with a call made
      --  directly from Expand_N_Indexed_Component.

      if Validity_Checking = Default then
      ...
      --  others choice (it's the list we are replacing!)
         ...
         begin
            ...
            if Comes_From_Source (N)
              and then not Range_Checks_Suppressed (Etype (Expr))
   ...
   begin
      ...
      --  Finalize the target of the assignment when controlled. We
      --  have 2 exceptions here:
      ...
      --  lhs is an uninitialized  temporary
      ...
      then
        ...
        and then not Comes_From_Source (Original_Node (Prefix (L)))
      then
         null;
