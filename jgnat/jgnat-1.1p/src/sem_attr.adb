...
package body Sem_Attr is
   ...
   --  Fold_Attribute in cases where this is more convenient.
   ...
   begin
      ...
      when Attribute_Max_Size_In_Storage_Elements =>
         if Esize_Known_By_Front_End (P_Type) then
            Fold_Uint (N,
      ...
      begin
         --  For discrete and fixed-point types, we use the RM_Size instead
         --  of Esize to get the proper RM semantics for the size attribute.
         ...
         --  if the size is known at front end compile time (this is true
         --  for all other primitive types, and for these types, Size and
         --  VADS_Size have the same meaning).

         elsif Esize_Known_By_Front_End (P_TypeA) then
            Fold_Uint (N, Esize (P_TypeA));
         end if;
      ...
      begin
         if Esize_Known_By_Front_End (P_TypeA) then
