...
package body Exp_Ch11 is
   ...
   begin
      ...
      while Present (Handler) loop
         ...
         if Hostparm.Java_VM then
            Prepend_Call_To_Handler (RE_Update_Exception);
   ...
   begin
      ...
      Scope_Loop : loop
         ...
         --  Task type case
            ...
            elsif Present (Task_Body_Procedure (Subp_Entity)) then
               Subp_Entity := Task_Body_Procedure (Subp_Entity);
               Set_Hlist;
               exit Scope_Loop;
            end if;
         end if;
   ...
   begin
      ...
      --  Very special case for No_Run_Time mode. Convert explicit raise
      --  of Program_Error into raise program error node (all other raises
      --  will get normal expansion and be disallowed).

      if No_Run_Time
        and then Present (Name (N))
        and then Nkind (Name (N)) = N_Identifier
        and then Entity (Name (N)) = Standard_Program_Error
      then
         Rewrite (N,
   ...
   begin
      if not Zero_Cost_Exceptions_On_Target then
         return;
      end if;
      ...
      --  The same consideration applies for No_Run_Time mode.

      if Restrictions (No_Exceptions) or No_Run_Time then
         return;
      end if;
...
--                    New_Occurrence_Of (Spec, Loc)),
   ...
   begin
      ...
      while Present (Ent) loop
         Next_Ent := Next (Ent);
         if Is_Eliminated (Entity (Prefix (Ent))) then
