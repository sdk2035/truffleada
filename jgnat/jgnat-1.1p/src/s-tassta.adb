...
--  Also used for Create_TSD, Destroy_TSD
...
package body System.Tasking.Stages is
   ...
   procedure Wakeup_Entry_Caller
     (Self_ID    : Task_ID;
      ...
      New_State  : Entry_Call_State) renames
     Initialization.Wakeup_Entry_Caller;

   procedure Cancel_Queued_Entry_Calls (T : Task_ID) renames
     Utilities.Cancel_Queued_Entry_Calls;

   procedure Abort_One_Task
     (Self_ID : Task_ID;
      T       : Task_ID) renames
     Utilities.Abort_One_Task;

   -----------------------
   -- Local Subprograms --
   -----------------------

   procedure Free is new Unchecked_Deallocation (String, Task_Image_Type);
   ...
   --  Different code is used at master completion, in
   --  Terminate_Dependents, due to a need for tighter synchronization
   --  with the master.
   ...
   begin
      ...
      Initialize_ATCB (Self_ID, State, Discriminants, P, Elaborated,
        Base_Priority, Task_Info, Size, Master, T, Success);
...
begin
   ...
   --  Initialize the Task_Wrapper procedure address used to compute the
   --  backtrace stop region for tasking program.

   SSL.Task_Wrapper_Address := Task_Wrapper'Code_Address;
