...
package body Xr_Tabls is
   ...
   begin
      ...
      Ref := new File_Record'(File_Length  => File_Name'Length,
                              File         => File_Name,
                              Dir          => null,
                              Lines        => null,
                              Visited      => Visited,
   ...
   begin
      case Ref_Type is
         when 'b' => Ref := Declaration.Body_Ref;
         when 'r' => Ref := Declaration.Ref_Ref;
         ...
         begin
            ...
            while Index <= Buffer'Last loop
               ...
               then
                  ...
                  if Buffer (Last - 1) /= GNAT.OS_Lib.Directory_Separator then
                     Append (Build_Dir, GNAT.OS_Lib.Directory_Separator);
                  end if;
               end if;
            ...
            while Index <= Buffer'Last loop
               ...
               then
                  ...
                  begin
                     ...
                     if S (S'First) /= Directory_Separator then
                        Append (Src_Dir, Build_Dir);
                     end if;

                     if S (S'Last) = Directory_Separator then
                        Append (Src_Dir, S & " ");
                     else
                        Append (Src_Dir, S & Directory_Separator & " ");
                     end if;
                  end;
               ...
               then
                  ...
                  begin
                     ...
                     if S (S'Last) = Directory_Separator then
                        ...
                        Append (Obj_Dir, S & Directory_Separator & " ");
                     end if;
                  end;
               end if;
   ...
   function Get_Type (Decl : Declaration_Reference) return String is
   begin
      return String'(1 => Ada.Characters.Handling.To_Upper (Decl.Decl_Type));
   end Get_Type;
   ...
   begin
      ...
      for J in 1 .. Osint.Nb_Dir_In_Src_Search_Path loop
         if Osint.Dir_In_Src_Search_Path (J)'Length = 0 then
            Ada.Strings.Unbounded.Append
              (Gnatls_Src_Cache,
               Osint.To_Host_Dir_Spec ("./", False).all & ' ');
         else
            Ada.Strings.Unbounded.Append
              (Gnatls_Src_Cache,
               Osint.To_Host_Dir_Spec (Osint.Dir_In_Src_Search_Path (J).all,
                                       False).all
               & ' ');
         end if;
      end loop;
      ...
      for J in 1 .. Osint.Nb_Dir_In_Obj_Search_Path loop
         if Osint.Dir_In_Obj_Search_Path (J)'Length = 0 then
            ...
            Ada.Strings.Unbounded.Append
              (Gnatls_Obj_Cache,
               Osint.To_Host_Dir_Spec (Osint.Dir_In_Obj_Search_Path (J).all,
                                       False).all
               & ' ');
         end if;
      end loop;
