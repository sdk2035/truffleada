...
package body Exp_Aggr is
   ...
   function Build_Record_Aggr_Code
     (N      : Node_Id;
      ...
      Obj    : Entity_Id := Empty)
      return   List_Id;
   --  N is an N_Aggregate or a N_Extension_Aggregate. Typ is the type of
   ...
   --  components if any. Obj is present in the object declaration case,
   --  it contains the Entity_Id of the object being declared.
   ...
   --    4. The array type of N does not follow the Fortan layout convention
   ...
   function Backend_Processing_Possible (N : Node_Id) return Boolean is
      ...
      --  Recursively checks that N is fully positional, returns true iff so.
      ...
      begin
         ...
         while Present (Expr) loop
            ...
            if (Nkind (Expr) = N_Aggregate and then Expansion_Delayed (Expr))
              or else (Nkind (Expr) = N_Qualified_Expression
                        and then Nkind (Expression (Expr)) = N_Aggregate
                        and then Expansion_Delayed (Expression (Expr)))
            then
               return False;
            end if;
   ...
   is
      ...
      begin
         ...
         then
            ...
            then
               return
                 Late_Expansion (Expr_Q, Etype (Expr_Q), Indexed_Comp, Flist);
            end if;
         end if;

         --  Now generate the assignment with no associated controlled
         --  actions since the target of the assignement may not have
         --  been initialized, it is not possible to Finaliuze it as
   ...
   begin
      ...
      if Parent_Kind = N_Qualified_Expression then
         ...
         declare
            Ancestor : Node_Id := Parent (Parent (N));

         begin
            Parent_Kind := Nkind (Ancestor);
            if Parent_Kind = N_Object_Declaration then
               Unc_Decl :=
                 not Is_Entity_Name (Object_Definition (Ancestor))
                 or else Has_Discriminants (
                   Entity (Object_Definition (Ancestor)))
      ...
      then
         ...
         Rewrite (N, New_Copy_Tree (Name (Parent (N))));
   ...
   begin
      ...
      Insert_Actions_After (N,
        Build_Record_Aggr_Code (Aggr, Typ, Occ, Obj => Obj));
   ...
   procedure Expand_Array_Aggregate (N : Node_Id) is
      ...
      --  conversion is possible only in some static cases. On return, if
      --  the aggregate was already positional or if the conversion was not
      --  possible, then True is returned. If the conversion is possible,
      --  then N is rewritten with the analyzed converted aggregate.
      ...
      procedure Convert_To_Positional (N : Node_Id) is
         ...
         --  into a list of values. This applies when convering a named
      ...
      function In_Place_Assign_OK return Boolean is
         ...
         begin
            ...
            --  entity information for the checks that follow.
            ...
            return Compile_Time_Known_Value (Comp)
              ...
              or else (Nkind (Comp) = N_Attribute_Reference
                        and then Safe_Component (Prefix (Comp)))

              or else (Nkind (Comp) in N_Binary_Op
                        and then Safe_Component (Left_Opnd  (Comp))
                        and then Safe_Component (Right_Opnd (Comp)))
   ...
   begin
      ...
      --  First try to convert to positional form. If this results in us
      --  not having an aggregate any more, then we are done with the
      --  analysis (it could be a string literal or an identifier for
      --  a temporary variable following this call.
      ...
      if Parent_Kind = N_Qualified_Expression then
         Parent_Kind := Nkind (Parent (Parent (N)));
      end if;
