...
package Checks is
   ...
   --  detected, then OK is False on exit, and Lo and Hi are not set.
   ...
   --  is not a discrete type, then the call has no effect, since validity
   --  is only an issue for discrete types. The effect of this call is to
