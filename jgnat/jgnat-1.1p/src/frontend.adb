...
begin
   ...
   if Opt.Config_File then
      ...
      Name_Len := 8;
      Source_gnat_adc := Load_Source_File (Name_Enter);
   ...
   then
      ...
      if Operating_Mode = Generate_Code then
         ...
         if not Debug_Flag_UU and then not Zero_Cost_Exceptions_On_Target then
            Collect_Garbage_Entities;
         end if;

         --  Remove entities from program that do not have any
         --  execution time references. For now, this analysis
         --  only covers subprogram entities.
