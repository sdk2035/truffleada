...
package body Sem_Ch10 is
      ...
      then
         if Is_RCI_Pkg_Spec_Or_Body (N) then
            ...
            Add_Stub_Constructs (N);
                 ...
                 and then Is_Shared_Passive (Defining_Unit_Name
                                              (Specification (Unit_Node))))
      ...
      then
         ...
         begin
            ...
            then
               ...
               --  Manually reset the True_Constant indication. This is needed
               --  because the assignment to the variable is generated by gigi
               --  and will not cause the flag to be reset.

               Set_Is_True_Constant (Elaboration_Entity (Spec_Id), False);
            end if;
         end;
      end if;
   ...
   procedure Analyze_With_Clause (N : Node_Id) is
      ...
      Saved_Style_Check : constant Boolean := Opt.Style_Check;
   ...
   begin
      ...
      --  we are compiling the internal unit as a main unit.

      if not Is_Internal_File_Name (Unit_File_Name (Current_Sem_Unit))
   ...
   begin
      if not Extensions_Allowed then
         Error_Msg_N ("`WITH TYPE` is a non-standard extension", N);

         if OpenVMS or Debug_Flag_M then
            Error_Msg_N
              ("\unit must be compiled with " &
               "'/'E'X'T'E'N'S'I'O'N'S'_'A'L'L'O'W'E'D qualifier", N);
         else
            Error_Msg_N
              ("\unit must be compiled with -gnatX flag", N);
         end if;
      end if;
      ...
      begin
         ...
         if Nkind (Parent (P)) = N_Defining_Program_Unit_Name then
            ...
            begin
               ...
               while Present (Parent_Comp) loop
            ...
            then
               ...
               if Tagged_Present (N) then
                  ...
                  then
                     ...
                     Set_Etype                    (Typ, Typ);
                     Set_Scope                    (Typ, P);
                     Init_Esize                   (Typ, System_Address_Size);
                     Init_Alignment               (Typ);
