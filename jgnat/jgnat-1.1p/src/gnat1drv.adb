...
begin
   ...
   begin
      ...
      --  We override the zero cost exceptions setting if set by switch

      if Opt.Zero_Cost_Exceptions_Set then
         Targparm.Zero_Cost_Exceptions_On_Target :=
           Opt.Zero_Cost_Exceptions_Val;
      end if;
      ...
      then
         ...
         Write_Str (" Copyright 1992-2000 Free Software Foundation, Inc.");
         Write_Eol;
      end if;
      ...
      then
         ...
         begin
            ...
            --  the subnunit case, because it is not a concern. Indeed it is
            --  is perfectly legal in Ada 95 to have a subunit with a name
            --  matching a package that does not need a body, and in Ada 83
            --  mode this is not a warning case.
      ...
      --  Do not lock Namet, Sinput, and Stringt when compiling for the Java VM
      --  since the Java VM backend uses these to store internal names.
      ...
      --  name for the library file output. This need not be done for the
      --  Java VM backend since Namet was not locked in the first place.
      ...
      if Opt.List_Representation_Info then
         List_Rep_Info;
      end if;
