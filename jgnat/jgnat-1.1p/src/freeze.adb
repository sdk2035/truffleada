...
package body Freeze is
   ...
   procedure Adjust_Size_For_Alignment (Typ : Entity_Id);
   --  Typ is a type that is being frozen. If no size clause is given,
   --  but a default size has been computed, then this default size is
   ...
   procedure Expand_Entry_Default (F : Entity_Id; Dbody : Node_Id);
   --  The subprogram created for a default expression of an entry or
   --  protected operation whose enclosing type has discriminants requires
   --  special processing. It has a formal parameter which is the target
   --  object in a call to the operation. References to the discriminant
   --  are transformed into references to the corresponding formal.
   ...
   --  mode. However, it never sets the flag if Debug_Info_Off is set.
   ...
   -------------------------------
   -- Adjust_Size_For_Alignment --
   -------------------------------

   procedure Adjust_Size_For_Alignment (Typ : Entity_Id) is
      Align : Uint;

   begin
      if not Has_Size_Clause (Typ)
        and then Esize_Known_By_Front_End (Typ)
      then
   ...
   procedure Check_Compile_Time_Size (T : Entity_Id) is
      ...
      function Static_Discriminated_Components (T : Entity_Id)
        return Boolean
      is
         Constraint : Elmt_Id;
               ...
               begin
                  ...
                  then
                     ...
                     if not Is_Constrained (T)
                       ...
                       and then not Esize_Known_By_Front_End (T)
                     then
                        return False;
                     else
                  ...
                  while Present (Comp) loop
                     if not Is_Type (Comp) then
                        if not Size_Known (Etype (Comp)) then
                           ...
                           if Esize_Known_By_Front_End (Etype (Comp)) then
                              Packed_Size :=
   ...
   begin
      ...
      elsif Comes_From_Source (T) or else Debug_Generated_Code then
   ...
   --------------------------
   -- Expand_Entry_Default --
   --------------------------

   procedure Expand_Entry_Default (F : Entity_Id; Dbody : Node_Id) is
      Loc   : constant Source_Ptr := Sloc (Dbody);
      New_F : constant Entity_Id := Make_Defining_Identifier (Loc, Chars (F));
      Ttyp  : constant Entity_Id := Scope (Scope (F));

   begin
      --  Formal parameter has type of the enclosing concurrent type.

      Set_Parameter_Specifications (Specification (Dbody),
        New_List (
          Make_Parameter_Specification (Loc,
            Defining_Identifier => New_F,
            Parameter_Type      => New_Occurrence_Of (Ttyp, Loc))));
   end Expand_Entry_Default;
   ...
   begin
      ...
      while Present (E) loop
         ...
         then
            ...
            declare
               Ent : Entity_Id;
               Nod : Node_Id := Parent (E);

            begin
               Ent := First_Entity (E);

               --  Patently wrong ??? E is the protected type. Lingering
               --  problem with default functions for protected operations.
               --  (see 6412-009).

               if Is_Overloadable (E) then
                  Make_Default_Expr_Functions (Nod, E);
                  Process_Default_Expressions (E, After);
               end if;
            end;
   ...
   begin
      ...
      --  Do not freeze a global entity within an inner scope. A call to
      --  subprogram E within some internal procedure (a stream attribute
      --  for example) might require freezing E, but the freeze node must
      --  appear in the same declarative part as E itself. The two-pass
      --  elaboration mechanism in gigi guarantees that E will be frozen
      --  before the inner call is elaborated. We exclude constants from
      --  this test, because deferred constants may be frozen early, and
      --  must be diagnosed (see e.g. 1522-005).
      ...
      if not Is_Type (E) then
            ...
            if not Is_Internal (E) then
               ...
               begin
                  ...
                  while Present (Formal) loop
                     ...
                     if Is_Private_Type (Etype (Formal))
                       and then No (Full_View (Base_Type (Etype (Formal))))
                       and then not Is_Generic_Type (Etype (Formal))
                       and then not Is_Derived_Type (Etype (Formal))
                     then
                        --  If the type of a formal is incomplete, subprogram
                        --  is being frozen prematurely. Within an instance
                        --  this is an artifact of our need to regard the end
                        --  of an instantiation as a freeze point. Otherwise
                     ...
                     if Is_Fat_C_Ptr_Type (Etype (Formal)) then
         ...
         --  Note: pragma Atomic[_Components] also sets Volatile[_Components]
         ...
         then
            ...
            then
               Error_Msg_N
                 ("stand alone atomic/volatile constant must be imported", E);
            end if;
         end if;
      ...
      --  Case of a type or subtype being frozen
               ...
               if E = Base_Type (E) then
                  ...
                  begin
                     if (Is_Packed (E) or else Has_Pragma_Pack (E))
                       ...
                       and then Esize_Known_By_Front_End (Ctyp)
                     then
                        Csiz := UI_Max (Get_RM_Size (Ctyp), 1);

                     elsif Component_Size (E) /= Uint_0 then
                        Csiz := Component_Size (E);

                     elsif not Esize_Known_By_Front_End (Ctyp) then
                        Csiz := Uint_0;
         ...
         then
            ...
            while Present (Comp) loop
               ...
               --  Processing for real components
               ...
               then
                  ...
                  --  non-default bit order case. We cannot do this any
                  --  earlier on, because there is no required order for
                  --  the component clause and the bit_order clause.

                  if Present (Component_Clause (Comp))
                    and then Reverse_Bit_Order (E)
                  then
                     declare
                        CFB : constant Uint    := Component_First_Bit (Comp);
                     ...
                     begin
                        ...
                        --  Case where field fits in one storage unit
                           ...
                           --  Here is where we fix up the Component_First_Bit
                           ...
                           --  The general rule is that the new starting bit
            ...
            if  Ekind (E) = E_Record_Type then
               ...
               if Present (Corresponding_Remote_Type (E)) then
                  Freeze_And_Append
                    (Corresponding_Remote_Type (E), Loc, Result);
               end if;
               ...
               while Present (Comp) loop
                  ...
                  if Has_Per_Object_Constraint (Comp)
                    and then not Is_Limited_Type (E)
                  then
         ...
         if Esize_Known_By_Front_End (E) then
            null;
            ...
            begin
               ...
               then
                  Init_Esize (E, 2 * System_Address_Size);
                  ...
                  Init_Esize (E, System_Address_Size);
               end if;
            end;
         end if;
      ...
      --  For imported subprogram in main unit, generate descriptor if
      --  we are in Propagate_Exceptions mode. Skip intrinsic subprograms
      --  and those with address clauses.

      if Is_Subprogram (E)
        and then Propagate_Exceptions
   ...
   exception
      when Cannot_Be_Static =>
         ...
         elsif Is_Exported (E) then
            Error_Msg_N
              ("& cannot be exported (local type is not constant)", E);

         --  Otherwise we have some kind of error (since the compiler is
         --  marking something as statically allocated which cannot be.
      ...
      while Present (Formal) loop
         if Present (Default_Value (Formal)) then
            ...
            --  Else construct the body of the function
               ...
               then
                  Expand_Entry_Default (Formal, Dbody);
               end if;
