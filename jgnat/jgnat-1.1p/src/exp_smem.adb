...
package body Exp_Smem is
   ...
   --  Insert a Shared_Memory_Read call for variable before node N

   procedure Add_Write_After (N : Node_Id);
   --  Insert a Shared_Memory_Write call for variable after the node
   ...
   begin
      if Present (Shared_Mem_Read_Proc (Ent)) then
         Insert_Action (N,
           Make_Procedure_Call_Statement (Loc,
             Name =>
               New_Occurrence_Of (Shared_Mem_Read_Proc (Ent), Loc),
             Parameter_Associations => Empty_List));
      end if;
   end Add_Read_Before;

   -----------------------------
   -- Add_Shared_Memory_Locks --
   -----------------------------

   procedure Add_Shared_Memory_Locks (N : Node_Id) is
      Loc   : constant Source_Ptr := Sloc (N);
   ...
   begin
      --  We have to add Shared_Mem_Lock and Shared_Mem_Unlock calls around
      ...
      Insert_Before_And_Analyze (Inode,
        Make_Procedure_Call_Statement (Loc,
          Name => New_Occurrence_Of (RTE (RE_Shared_Mem_Lock), Loc),
      ...
      Insert_Before_And_Analyze (Inode,
        Make_Procedure_Call_Statement (Loc,
          Name => New_Occurrence_Of (Shared_Mem_Read_Proc (Obj), Loc)));

      --  Now insert the Unlock call after

      Insert_After_And_Analyze (Inode,
        Make_Procedure_Call_Statement (Loc,
          Name => New_Occurrence_Of (RTE (RE_Shared_Mem_Unlock), Loc),
          Parameter_Associations => New_List (
            Make_String_Literal (Loc, Vnm))));
      ...
      if Nkind (N) = N_Procedure_Call_Statement then
         Insert_After_And_Analyze (Inode,
           Make_Procedure_Call_Statement (Loc,
             Name => New_Occurrence_Of (Shared_Mem_Assign_Proc (Obj), Loc)));
      end if;

   end Add_Shared_Memory_Locks;
   ...
   begin
      if Present (Shared_Mem_Assign_Proc (Ent)) then
         Insert_After_And_Analyze (Insert_Node,
           Make_Procedure_Call_Statement (Loc,
             Name =>
               New_Occurrence_Of (Shared_Mem_Assign_Proc (Ent), Loc),
             Parameter_Associations => Empty_List));
      end if;
   end Add_Write_After;
   ...
   ---------------------------
   -- Make_Shared_Mem_Procs --
   ---------------------------

   procedure Make_Shared_Mem_Procs (N : Node_Id) is
      Loc : constant Source_Ptr := Sloc (N);
   ...
   --  Start of processing for Make_Shared_Mem_Procs
              ...
              Statements => New_List (
               ...
               --  if S /= null then
                ...
                Make_Implicit_If_Statement (N,
                  ...
                  Then_Statements => New_List (Atr))))));

      Set_Is_Shared_Passive      (Ent, True);
      Set_Shared_Mem_Assign_Proc (Ent, Assign_Proc);
      Set_Shared_Mem_Read_Proc   (Ent, Read_Proc);
   end Make_Shared_Mem_Procs;
