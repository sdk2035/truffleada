...
package body Style is
   ...
   --  format of comments. Comments that are not at the start of a line merely
   --  require at least one space after the second minus, there is no other
   --  requirement.

   --  For comments at the start of a line, either two blanks appear after the
   --  second minus, or a vertical bar appears after the minus (this allows
   --  gnatprep output to be properly processed, or as special cases, a row
   --  of minuses, or a line starting with two blanks and a minus and ending
   --  with a blank and two minuses is permitted. To see the reason for these
   --  special exceptions, look at the box that precedes this procedure!
   ...
   begin
      ...
      --  Case of a comment that is at the start of a line
         ...
         --  some exceptions that we permit.
            ...
            --  Case of --! is OK (gnatprep comments)
   ...
   --  In check end labels mode (-gnatye), we always require the name of a
   --  subprogram or package to be present on the END, so this is an error.
