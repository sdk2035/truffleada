...
package body Exp_Ch4 is
   ...
   begin
      ...
      case List_Length (Opnds) is
         when 2 =>
            if Typ1 = Standard_Character then
               if Typ2 = Standard_Character then
                  R := RE_Str_Concat_CC;
               elsif Typ2 = Standard_String then
                  R := RE_Str_Concat_CS;
               else
         ...
         when 3 => R := RE_Str_Concat_3;
         when 4 => R := RE_Str_Concat_4;
         when 5 => R := RE_Str_Concat_5;
      ...
      then
         ...
         Insert_Action (N,
           Make_Object_Declaration (Loc,
             ...
             Object_Definition   => Subtype_Mark (Expression (N)),
            ...
            --  Case of initialization procedure present, must be called
                  ...
                  if Nkind (Parent (N)) = N_Assignment_Statement then
                     ...
                     begin
                        if Is_Entity_Name (Nam) then
                           ...
                           Decl := Build_Task_Image_Decl (Loc, T, T);
                        end if;
                     end;
               ...
               if Has_Task (T) then
                  ...
                  begin
                     ...
                     Prepend (Decl, Declarations (Last (L)));
                     Insert_Actions (N, L);
                  end;
   ...
   begin
      ...
      --  Check all subscripts if in full validity checking mode

      if Validity_Checking = Full then
   ...
   begin
      ...
      elsif Is_Boolean_Type (Typ1) then
         Adjust_Condition   (Left_Opnd (N));
         Adjust_Condition   (Right_Opnd (N));
         Set_Etype (N, Standard_Boolean);
         Adjust_Result_Type (N, Typ);
   ...
   begin
      ...
      then
         ...
         --  Instead of reanalyzing the node we set entity and type directly.
         --  This avoids anomalies when the replacement is done in an instance
         --  and is epsilon more efficient.
      ...
      --  Otherwise, analyze the expression using the rem circuit
   ...
   --  Note: this routine is also used for the mod case
   ...
   begin
      ...
      --  minus one. Gigi does not handle this case correctly, so we do
      --  it in the front end.
            ...
            if not Is_Constrained (Target_Type) then
               if Has_Discriminants (Operand_Type) then
                  ...
                  while Present (Indx) loop
                     if Base_Type (Etype (Indx)) =
                        Base_Type (Etype (N_Ix))
                     then
                        Append_To (Cons,
                          New_Occurrence_Of (Etype (Indx), Loc));
                     else
                        --  Need to convert the bounds explicitly.
                          ...
                          Make_Range (Loc,
                            Low_Bound  =>
                              Unchecked_Convert_To (Etype (N_Ix),
                                New_Copy_Tree
                                  (Type_Low_Bound (Etype (Indx)))),
                            High_Bound =>
                              Unchecked_Convert_To (Etype (N_Ix),
                                New_Copy_Tree
                                  (Type_High_Bound (Etype (Indx))))));
                     end if;
