...
package body Sem_Dist is
   ...
   begin
      ...
      elsif Nkind (U) = N_Package_Instantiation then
         Exp  := Instance_Spec (U);
   ...
   begin
      ...
      while Current_Declaration /= Empty loop
         ...
         then
            if
              Defining_Unit_Name (Specification (Current_Declaration)) = E
            then
               return Result;
            end if;
   ...
   begin
      ...
      if Ekind (New_Type) = E_Record_Type then
         RAS_Type := New_Type;

      else
         Attribute_Subp := TSS (RAS_Type, Name_uRAS_Access);

         if No (Attribute_Subp) and then Expander_Active then
   ...
   begin
      ...
      if No (Deref_Proc) and Expander_Active then
