...
package body Sem_Ch12 is
               ...
               begin
                  while Enclosing_Master /= Standard_Standard loop
                     ...
                     then
                        ...
                        --  is void in the formal part of a a generic subp.
                        ...
                        elsif Is_Task_Type (Enclosing_Master)
                          and then
                            Present (Task_Body_Procedure (Enclosing_Master))
                        then
                           Delay_Descriptors
                            (Task_Body_Procedure (Enclosing_Master));
                           Set_Delay_Cleanups
                            (Task_Body_Procedure (Enclosing_Master));
                        end if;
   ...
   begin
      ...
      if Present (Gen_Body_Id) then
         ...
         --  Case where instantiation is not a library unit
            ...
            --  on for any GNAT library or run-time library code. All such
  ...
  is
      ...
      begin
         ...
         elsif Nkind (B) = N_Package_Body_Stub then
   ...
   begin
      ...
      then
         ...
         elsif Nkind (Enc_G) = N_Package_Body_Stub then
            E_G_Id :=
   ...
   is
      ...
      begin
         ...
         --  resolved, e.g. in an enclosing instantiations.

         if Present (Entity (Id))
           and then Scope (Entity (Id)) = Scop
         then
            return Entity (Id);
   ...
   begin
      ...
      if Nkind (Gen_Id) = N_Selected_Component then
         ...
         --  an instance in the current context.
   ...
   begin
      ...
      --  OUT not present
         ...
         if Present (Actual) then
            ...
            --  If the actual is an aggregate, perform name resolution on its
            --  components (the analysis of an aggregate does not do it) to
            --  capture local names that may be hidden if the generic is a
            --  child unit.
            ...
            if Nkind (Actual) = N_Aggregate then
               declare
                  Typ : Entity_Id
                    := Etype (Defining_Identifier (Analyzed_Formal));
               begin
                  Resolve_Aggregate (Actual, Get_Instance_Of (Typ));
               end;
            end if;
